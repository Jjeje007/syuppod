...
# OK all good normally this is version 0.1 final 
# As of 10/08/2019 
...
class GitTracking:
    """Git tracking class."""
    def __init__(self, interval, repo, stateinfo, log, gitlog):
    ...
    def dopull(self):
        ...
        try:
            ...
            mylogfile = processlog.dolog(self.gitlog)
            mylogfile.setLevel(processlog.logging.INFO)
            mylogfile.info('##################################')
            for line in gitlog.splitlines():
                mylogfile.info(line)
            self.log.debug(f'Successfully wrote git pull log to \'{self.gitlog}\'.')
